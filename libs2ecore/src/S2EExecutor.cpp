...
void S2EExecutor::registerCpu(S2EExecutionState *initialState, CPUX86State *cpuEnv) {
    std::cout << std::hex
              << "Adding CPU (addr = " << cpuEnv
              << ", size = 0x" << sizeof(*cpuEnv)
              << " [0x" << offsetof(CPUX86State, eip) << "] as symbolic"
              << " [0x" << sizeof(CPUX86State) - offsetof(CPUX86State, eip) << "] as concrete )"
              << std::dec << '\n';
...
S2EExecutor::StatePair S2EExecutor::fork(ExecutionState &current, klee::ref<Expr> condition, bool isInternal,
    ...
    if (!dyn_cast<klee::ConstantExpr>(condition)) {
        ...
        g_s2e->getCorePlugin()->onStateForkDecideCondition.emit(currentState, condition, &forkOk);
    }
