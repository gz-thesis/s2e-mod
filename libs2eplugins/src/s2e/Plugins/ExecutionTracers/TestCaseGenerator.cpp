...
void TestCaseGenerator::assembleTestCaseToFiles(const ConcreteInputs &inputs, const std::string &prefix,
    ...
    foreach2 (it, files.begin(), files.end()) {
        ...
        /* only put the testcase file into default directory when output directory not specified */


        getWarningsStream() << m_pathTestCases << "\n";
        if( m_pathTestCases.compare("") == 0 ) {
            outputFileName = s2e()->getOutputFilename(ss.str());
        } else {
            llvm::SmallString<128> filePath(m_pathTestCases);
            llvm::sys::path::append(filePath, ss.str());
            outputFileName = filePath.str();
        }
