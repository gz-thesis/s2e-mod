...
void ForkLimiter::initialize() {
    ...
    m_overall_limit = s2e()->getConfig()->getInt(getConfigKey() + ".overallForkCount", 10, &ok);
    if ((int) m_limit != -1 || (int)m_overall_limit != -1) {
        if (m_detector) {
    ...
    m_timerTicks = 0;
    m_overall_forkCount = 0;
}

void ForkLimiter::onStateForkDecide(S2EExecutionState *state, bool *doFork) {
    if((int) m_overall_limit != -1 && m_overall_forkCount > m_overall_limit) {
        *doFork = false;
        return;
    }
